...
public class OrderConsumer extends Thread {
    ...
    private ActionHandler orderActionHandler;
    ...
    public OrderConsumer(@Value("${kafka.orders.server}") String server,
        ...
        orderActionHandler = new OrderActionHandler(dataSource);
    }
    ...
    public void run() {
        while (true) {
            ...
            for (ConsumerRecord record : records) {
                try {
                    orderActionHandler.handle(record.key().toString(), record.value().toString());
                } catch (Exception e) {
